    ...
    for object in objects:
        ...
        if object.started == 0 and f"{current_time.hour:02d}:{current_time.minute:02d}" == f"{object.time.hour}:{object.time.minute}":
            if object.telegram_id != 0:
                if object.relhab:
                    bot.send_message(object.telegram_id,
                                     f"Пора выполннять привычку {object.action} на протяжении {object.time_to_act} секунд в {object.place}, а затем {object.relhab}")
                elif object.reward:
                    bot.send_message(object.telegram_id,
                                     f"Пора выполннять привычку {object.action} в {object.place}, а затем наградиться себя {object.reward}")
